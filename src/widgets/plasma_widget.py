...
class PlasmaWidget(Gtk.Window):
    ...
    def __init__(self, app: Gtk.Application):
        ...
        if display:
            ...
            try:
                ...
                if renderer:
                    ...
                    if gpu_available:
                        print(f"[PLASMA] ✓ GPU-accelerated compositing: {renderer_type}")
                    else:
                        print(f"[PLASMA] Rendering backend: {renderer_type}")
            except AttributeError:
                # X11 display doesn't have get_renderer()
                display_name = display.get_name() if hasattr(display, 'get_name') else 'X11'
        ...
        # Also add drag controller to window itself
        window_drag_controller = Gtk.GestureDrag()
        window_drag_controller.set_button(1)
        window_drag_controller.connect('drag-begin', self.on_drag_begin)
        window_drag_controller.connect('drag-update', self.on_drag_update)
        window_drag_controller.connect('drag-end', self.on_drag_end)
        self.add_controller(window_drag_controller)
        ...
        self._drag_update_interval = 0.005  # ~200fps max update rate for very smooth dragging
        ...
        self._current_base_color: Tuple[float, float, float] = (0.2, 0.4, 0.8)  # Start with idle color
        self._target_base_color: Tuple[float, float, float] = (0.2, 0.4, 0.8)  # Target color
        self._color_transition_progress: float = 1.0  # 0.0 = start, 1.0 = complete
        self._color_transition_duration: float = 2.0  # Duration in seconds for color transition (smooth, gradual transitions)
        self._color_transition_start_time: float = 0.0
        self._last_state_change_time: float = 0.0  # Track when state last changed for debouncing
        self._needs_redraw: bool = True  # Track if redraw is needed (performance optimization)
    ...
    def _get_base_color(self, state: str) -> Tuple[float, float, float]:
        ...
        if self._color_transition_progress < 1.0:
            ...
            base_r = self._current_base_color[0] + (self._target_base_color[0] - self._current_base_color[0]) * eased_t
            base_g = self._current_base_color[1] + (self._target_base_color[1] - self._current_base_color[1]) * eased_t
            base_b = self._current_base_color[2] + (self._target_base_color[2] - self._current_base_color[2]) * eased_t
        else:
    ...
    def _draw_idle_plasma(self, cr, center_x: float, center_y: float, radius: float, width: int, height: int):
        ...
        pattern2.add_color_stop_rgba(1.0, r * 0.2, g * 0.2, b * 0.2, 0.0)  # Fade to transparent
    ...
    def _draw_listening_plasma(self, cr, center_x: float, center_y: float, radius: float, width: int, height: int):
        ...
        pattern2.add_color_stop_rgba(1.0, r * 0.3, g * 0.3, b * 0.3, 0.0)  # Fade to transparent
        ...
        pattern3 = cairo.RadialGradient(
            center_x, center_y, 0,
            center_x, center_y, radius * 0.4 * pulse2
        )
    ...
    def _draw_processing_plasma(self, cr, center_x: float, center_y: float, radius: float, width: int, height: int):
    ...
    def _draw_responding_plasma(self, cr, center_x: float, center_y: float, radius: float, width: int, height: int):
    ...
    def _draw_particles(self, cr, center_x: float, center_y: float, radius: float):
        ...
        for particle in self._particles:
            ...
            x = center_x + dist * math.cos(particle['angle'] + self._animation_time * particle['speed'])
            y = center_y + dist * math.sin(particle['angle'] + self._animation_time * particle['speed'])
            
            # Get color based on state
            r, g, b = self._get_base_color(self._state)
            
            # Particle size varies with animation
            size = particle['size'] * (1.0 + 0.3 * math.sin(self._animation_time * 2 + particle['phase']))
    ...
    def _draw_waves(self, cr, center_x: float, center_y: float, radius: float, max_radius: float):
        """Draw wave/ripple effects"""
        # Add waves based on state activity
        if self._state == 'processing':
            # Intense waves for processing - constrain to max_radius
            for i in range(3):
                wave_radius = radius * (1.2 + i * 0.3 + 0.2 * math.sin(self._animation_time * 4 + i))
                # Ensure waves don't exceed the margin
                wave_radius = min(wave_radius, max_radius)
                alpha = 0.15 / (i + 1) * (1.0 - (wave_radius - radius * 1.2) / (radius * 0.6))
                if alpha > 0:
                    r, g, b = self._get_base_color(self._state)
    ...
    def on_button_press(self, controller: Gtk.GestureClick, n_press: int, x: float, y: float) -> None:
    ...
    def on_button_release(self, controller: Gtk.GestureClick, n_press: int, x: float, y: float) -> None:
        """Handle mouse button release"""
        self._resizing = False
    
    def on_drag_begin(self, gesture: Gtk.GestureDrag, start_x: float, start_y: float) -> None:
        """Handle drag begin - start window dragging"""
        ...
        # Get current window position - use root coordinates immediately (fast, no blocking)
        # We'll get the actual position asynchronously if needed, but start with root coords
        try:
            sequence = gesture.get_current_sequence()
            if sequence:
                event = gesture.get_last_event(sequence)
                if event:
                    # Get root coordinates of the click immediately (no blocking)
                    root_x = event.get_x_root()
                    root_y = event.get_y_root()
                    # Calculate window position: root position minus relative click position
                    # This is instant and accurate enough to start dragging immediately
                    self._window_start_x = int(root_x - start_x)
                    self._window_start_y = int(root_y - start_y)
                else:
                    self._window_start_x = 0
                    self._window_start_y = 0
            else:
        ...
        # Optionally get actual position asynchronously in background (non-blocking)
        # This helps if the window was moved by something else, but doesn't block dragging
        def get_actual_position_async():
            try:
        ...
        # Get actual position in background (non-blocking)
        GLib.idle_add(get_actual_position_async)
        
        # Store drag start position (relative to widget)
    ...
    def on_drag_update(self, gesture: Gtk.GestureDrag, offset_x: float, offset_y: float) -> None:
        """Handle drag update - move window"""
        if not self._dragging:
            return
        
        # Calculate new window position directly from offset (no rate limiting)
        new_x = int(self._window_start_x + offset_x)
        new_y = int(self._window_start_y + offset_y)
        
        # Skip only if position hasn't changed at all
        ...
        # Cache the xid to avoid repeated lookups
        ...
        # Move window using X11 - kill previous process if still running to avoid queue
        if self._cached_xid is not None:
            try:
                # Kill previous subprocess if it's still running (prevents queue buildup)
                # But don't wait for it - just fire and forget the new position
                if self._drag_process is not None:
                    try:
                        # Quick check if still running, kill if so
                        if self._drag_process.poll() is None:  # Still running
                            self._drag_process.terminate()
                    except:
                        pass
                
                # Use xdotool with Popen - completely detached, no waiting
                # Close file descriptors immediately to avoid blocking
                self._drag_process = subprocess.Popen(
                    ['xdotool', 'windowmove', str(self._cached_xid), str(new_x), str(new_y)],
                ...
                # Update tracked position immediately (don't wait for subprocess)
                ...
                try:
                    if self._drag_process is not None:
                        try:
                            if self._drag_process.poll() is None:
                                self._drag_process.terminate()
                        except:
                            pass
                    self._drag_process = subprocess.Popen(
                        ['wmctrl', '-i', '-r', str(self._cached_xid), '-e', f'0,{new_x},{new_y},-1,-1'],
                        stdout=subprocess.DEVNULL,
                        stderr=subprocess.DEVNULL,
                        stdin=subprocess.DEVNULL,
                        close_fds=True,
    ...
    def on_drag_end(self, gesture: Gtk.GestureDrag, offset_x: float, offset_y: float) -> None:
        ...
        if hasattr(self, '_last_move_x'):
            delattr(self, '_last_move_x')
        if hasattr(self, '_last_move_y'):
            delattr(self, '_last_move_y')
        # Clear cached xid (window might have moved to different display)
        if hasattr(self, '_cached_xid'):
            delattr(self, '_cached_xid')
        # Cleanup process
        if self._drag_process is not None:
            try:
                self._drag_process.terminate()
    ...
    def on_motion(self, controller: Gtk.EventControllerMotion, x: float, y: float) -> None:
        ...
        if self._resizing:
            ...
            new_size = max(
                self.MIN_SIZE,
                min(self.MAX_SIZE, self._resize_start_width + delta)
            )
    ...
    def start_animation(self):
        ...
        def animate():
            ...
            if self._color_transition_progress < 1.0:
                ...
                self._color_transition_progress = min(1.0, elapsed / self._color_transition_duration)
                ...
                if abs(self._color_transition_progress - old_progress) > 0.01:  # 1% change threshold
                    needs_update = True
            ...
            if abs(opacity - current_opacity) > 0.01:  # Only update if significant change
        ...
        def animate_with_adaptive_rate():
            ...
            if self._state == 'idle' and self._color_transition_progress >= 1.0:
                interval = 33  # 30fps for idle state (still animated, but saves resources)
            else:
        ...
        self._animation_id = GLib.timeout_add(33, animate_with_adaptive_rate)  # 30fps initially
    ...
    def set_state(self, state: str):
        ...
        if state in ['idle', 'listening', 'processing', 'responding']:
            ...
            if self._color_transition_progress < 1.0:
                ...
                eased_t = self._ease_in_out_logarithmic(self._color_transition_progress)
                current_r = self._current_base_color[0] + (self._target_base_color[0] - self._current_base_color[0]) * eased_t
                current_g = self._current_base_color[1] + (self._target_base_color[1] - self._current_base_color[1]) * eased_t
                current_b = self._current_base_color[2] + (self._target_base_color[2] - self._current_base_color[2]) * eased_t
    ...
    def set_agent_color(self, color: Optional[Tuple[float, float, float]], intensity: float = 0.5):
        ...
        try:
            ...
            if surface and isinstance(surface, GdkX11.X11Surface):
                ...
                if hasattr(self, '_window_x') and hasattr(self, '_window_y'):
                    subprocess.Popen(
                        ['xdotool', 'windowmove', str(xid), str(self._window_x), str(self._window_y)],
                        stdout=subprocess.DEVNULL,
                        stderr=subprocess.DEVNULL
                    )
        except Exception:
